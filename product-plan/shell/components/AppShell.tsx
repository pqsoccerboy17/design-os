...
function useSidebarState(config: SidebarConfig) {
  const [intentState, setIntentState] = useLocalStorage<
    "collapsed" | "expanded"
  >("yourco-sidebar-state", config.defaultExpanded ? "expanded" : "collapsed");
    ...
    >
      ...
      <aside
        className={`
          ...
          transition-all duration-200 ease-out ${sidebarWidth}
          ${peekShadowClass}
      ...
      >
        ...
        <div className="flex items-center h-14 px-4 border-b border-stone-200 dark:border-stone-800">
          ...
          >
            <div className="w-8 h-8 rounded-xl bg-amber-500 flex items-center justify-center shadow-sm group-hover:shadow-md transition-shadow">
              <span className="text-white font-bold text-sm">Y</span>
            </div>
            {showLabels && (
              <span className="font-semibold text-lg transition-opacity duration-150">
                YourCo
              </span>
            )}
          </button>
