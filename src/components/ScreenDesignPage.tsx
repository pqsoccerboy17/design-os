...
export function ScreenDesignFullscreen() {
  ...
  // Load screen design component using module-level cache
  const ScreenDesignComponent = useMemo(() => {
    if (!sectionId || !screenDesignName) return null;
    const cacheKey = `${sectionId}/${screenDesignName}`;

    // Return cached component if available
    if (screenDesignCache.has(cacheKey)) {
      return screenDesignCache.get(cacheKey)!;
    }
    ...
    const LazyComponent = React.lazy(async () => {
      try {
        ...
        return {
          default: () => <div>Invalid screen design: {screenDesignName}</div>,
        };
      } catch (e) {
    ...
    screenDesignCache.set(cacheKey, LazyComponent);
  ...
  const AppShellComponent = useMemo(() => {
    ...
    if (sectionId && !sectionUsesShell(sectionId)) {
      console.log(
        "[ScreenDesignFullscreen] Section configured to not use shell",
      );
      return null;
    }
    ...
    const cacheKey = `shell/${sectionId || "default"}`;

    // Return cached component if available
    if (appShellCache.has(cacheKey)) {
      return appShellCache.get(cacheKey)!;
    }
    ...
    // Create and cache the lazy component
    const LazyShell = React.lazy(async () => {
      try {
        const module = (await loader()) as Record<string, unknown>;
        const ShellComponent = (module?.default ||
          module?.AppShell) as React.ComponentType<Record<string, unknown>>;

        if (typeof ShellComponent !== "function") {
          console.warn(
            "[ScreenDesignFullscreen] AppShell does not have a valid export",
          );
          return {
    ...
    appShellCache.set(cacheKey, LazyShell);
    return LazyShell;
