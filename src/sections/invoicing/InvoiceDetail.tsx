...
const activities = data.invoiceActivities.filter(a => a.invoiceId === invoice.id)
          ...
          <Card className="border-stone-200 dark:border-stone-700">
            <CardHeader className="border-b border-stone-200 dark:border-stone-700">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm text-stone-500 dark:text-stone-400">From</p>
                ...
                <div className="text-right">
                  <p className="text-sm text-stone-500 dark:text-stone-400">Bill To</p>
                  <p className="font-semibold text-stone-900 dark:text-stone-100">
                    {clientNames[invoice.clientId]}
                  </p>
                </div>
            ...
            <CardContent className="pt-6">
              ...
              <div className="grid grid-cols-4 gap-4 mb-6 pb-6 border-b border-stone-200 dark:border-stone-700">
                <div>
                  <p className="text-xs text-stone-500 dark:text-stone-400">Invoice Number</p>
                  <p className="font-medium text-stone-900 dark:text-stone-100">{invoice.invoiceNumber}</p>
                </div>
                <div>
                  <p className="text-xs text-stone-500 dark:text-stone-400">Issue Date</p>
                  <p className="font-medium text-stone-900 dark:text-stone-100">
                    {new Date(invoice.issueDate).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-stone-500 dark:text-stone-400">Due Date</p>
                  <p className="font-medium text-stone-900 dark:text-stone-100">
                    {new Date(invoice.dueDate).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-stone-500 dark:text-stone-400">Amount Due</p>
              ...
              {invoice.notes && (
                <div className="mt-6 pt-4 border-t border-stone-200 dark:border-stone-700">
                  <p className="text-sm text-stone-500 dark:text-stone-400 mb-1">Notes</p>
                  <p className="text-sm text-stone-700 dark:text-stone-300">{invoice.notes}</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          {payment && (
            <Card className="border-lime-200 dark:border-lime-800 bg-lime-50 dark:bg-lime-900/20">
              ...
              <CardContent className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-lime-700 dark:text-lime-400">Amount</span>
                  <span className="font-medium text-lime-900 dark:text-lime-200">{formatCurrency(payment.amount)}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-lime-700 dark:text-lime-400">Method</span>
                  <span className="font-medium text-lime-900 dark:text-lime-200 uppercase">{payment.paymentMethod}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-lime-700 dark:text-lime-400">Reference</span>
                  <span className="font-medium text-lime-900 dark:text-lime-200 font-mono text-xs">{payment.reference}</span>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="border-stone-200 dark:border-stone-700">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-stone-500 dark:text-stone-400">Client</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-stone-500 dark:text-stone-400">Payment Terms</p>
                  <p className="text-sm font-medium text-stone-900 dark:text-stone-100">NET 30</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                {activities.map((activity, _index) => (
                  <div key={activity.id} className="flex gap-3">
                    <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${
                      activity.type === 'paid' ? 'bg-lime-100 text-lime-600 dark:bg-lime-900/30 dark:text-lime-400' :
                      activity.type === 'sent' ? 'bg-sky-100 text-sky-600 dark:bg-sky-900/30 dark:text-sky-400' :
